...
my %fs_prefs = (
	...
	min_upload			=> 0,
	motd				=> '',
	motdfile			=> '$IRSSI/fserve.motd', #message that is displayed below the fserve instructions
	offline_message			=> 'Please try agains in a few minutes.', # is displayed when someone wants to enter disabled fserve
	queuefile			=> '$IRSSI/fserve.queue',
	recache_interval		=> 3607,
);

my %fs_queue_defaults = (
	channels			=> '#bookz', # change to appropriate channel
	content				=> 'Mostly EPUB Files', # short message that is displayed as the content field on the announce
	ctcp_only			=> 1,
	custom_notice			=> 1,
	...
	nice				=> 0,
	note				=> '',
	notify_interval			=> 300, # display the announce message every 5 minutes
	notify_on_join			=> 1, # display the annouce message when joining channel
	queue_priority			=> "", 
	request				=> "",
	restricted_level		=> 0,
	root_dir			=> '/home/pi/ebooks', # change to correct directory, ensure valid permissions!
	servers				=> 'Undernet', # change to correct server
	speed_warnings			=> 1,
	trigger				=> '!epuber', # change to correct trigger, in this case the IRC nick
	user_slots			=> 3,
);
